...
if __name__ == '__main__':
    ...
    def one_step_parallel(psis, U_ops, zeropsi, rank,pool):
        t_start=time.time()
        new_psis=pool.starmap(poolscript.apply_U,product(psis,U_ops))
        print("parallel computation takes {} seconds".format(time.time()-t_start))
        t_start=time.time()
        if len(new_psis) > rank:
            # mat = overlap(new_psis)
            # This is a bit awkward, needing to convert from a list of arrays to a 2d np array in this way, but it's the
    ...
    def lowrank_sim(rank, run_lowrank):
        ...
        if run_lowrank:
            K = len(c_op_list)
            print(2*K*rank)
            pool = multiprocessing.get_context("spawn").Pool(processes=2 * K * rank)
            loop_start = time.time()
            psis = [psi0]
            s_rank = []
            psi_times = []
            ...
            for j in tqdm(range(steps)):
                ...
                psis = one_step_parallel(psis, U_ops, zeropsi, rank,pool)
            pool.close()
            pool.join()
